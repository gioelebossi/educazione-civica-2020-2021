const alcoolCalc = (G : number, V : number, P : number, K : number) => {
    return (G * V * 0.008 * 1.055) / (P * K);
}

document.querySelector('#calc').addEventListener('click', () => {
    const G = parseFloat((document.querySelector('input[name=G]') as HTMLInputElement).value);
    const V = parseFloat((document.querySelector('input[name=V]') as HTMLInputElement).value);
    const P = parseFloat((document.querySelector('input[name=P]') as HTMLInputElement).value);
    const K = (document.querySelector('#m') as HTMLInputElement).checked ? 0.73 : 0.66;
...
document.querySelector('#move').addEventListener('click', () => {
    ...
    if(count < 5) document.querySelector('#moveText').innerHTML = "˅";
    (document.querySelector('#move') as HTMLAnchorElement).href = `#section${count}`;
    if(count == 5) count = 0;
});